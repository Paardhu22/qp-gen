...
Pre-deploy verification for the critical fix round (Clusters AΓÇôD) + the
...
### 0.1 Auth-store decision ΓÇö RESOLVED

**Login, register, and password reset all read/write the SAME column**
ΓÇö `account.password`, in the table created by the (originally
Prisma-authored) better-auth schema but now owned by Django via the
`apps.accounts.models.Account` ORM model.

Trace recorded definitively in `FIX_REPORT.md` ┬º0.0:

* FE `signIn.email` / `signUp.email` / `requestPasswordReset` /
  `resetPassword` POST to `${NEXT_PUBLIC_API_BASE_URL ||
  "http://localhost:8000"}/api/auth/<endpoint>` ΓÇö the **Django**
  origin, never the Next.js origin.
* `LoginView` ΓåÆ `authenticate_user` ΓåÆ `Account.check_password`
  against `account.password`.
* `RegisterView` ΓåÆ `register_user` ΓåÆ `account.set_password` ΓåÆ
  `account.save(update_fields=["password"])`.
* `ResetPasswordView` ΓåÆ `consume_reset_token` ΓåÆ
  `account.set_password(new_password)` ΓåÆ
  `account.save(update_fields=["password"])`.
...
### 0.2 Live verification ΓÇö what actually passed

Captured against the running Django backend during the closeout:

- [x] Register a fresh `welcome.test+<ts>@gmail.com` via HTTP ΓåÆ **201**.
- [x] Welcome email lands in the locmem outbox with subject
      `"Welcome to qp-gen"`.
- [x] Login with the registered password ΓåÆ **200** + token pair.
- [x] `POST /api/auth/forgot-password` ΓåÆ **200** (generic message).
- [x] Reset email lands in the outbox with subject `"Reset your qp-gen
      password"`. Body contains a reset link with `?token=<64 hex>`.
- [x] `POST /api/auth/reset-password` with the captured token ΓåÆ
      **200** `{success: true}`.
- [x] Login with the OLD password ΓåÆ **401** (correct rejection).
- [x] Login with the NEW password ΓåÆ **200** + token pair.
- [x] Stored hash format = `pbkdf2_sha256$720000$ΓÇª` ΓÇö Django
...
### 0.3 Bug caught during closeout ΓÇö FIXED

**Symptom**: `POST /api/auth/reset-password` with a real token ΓåÆ
...
ΓåÆ `TypeError`). `verification.expiresAt` is `TIMESTAMP WITHOUT TIME
...
- [ ] Editor: Date field in header ΓåÆ enable ΓåÆ exported PDF + DOCX
      both render the date.
- [ ] Editor: paste a clipboard image; verify it renders with resize
      handles and can be dragged.
- [ ] Editor: empty paper (only an unedited "Enter question hereΓÇª"
      block) ΓåÆ click Generate Answer Script ΓåÆ backend returns a clear
      400 with the empty-paper message (no hallucinated answer).
- [ ] Account isolation: sign in as user A ΓåÆ tray has items ΓåÆ sign
      out ΓåÆ sign in as user B ΓåÆ tray is empty.
- [ ] Account isolation: open in a private/clean browser profile ΓåÆ
      register a fresh account ΓåÆ tray is empty on first paint.
...
|-----|----------|---------|---------|
| `DATABASE_URL` | yes | Postgres connection string with `pgvector` extension | `postgres://ΓÇª?sslmode=require` |
...
| `OPENAI_API_KEY` | yes | Generation + embeddings | `sk-ΓÇª` |
...
| `OPENAI_VISION_MODEL` | optional | Vision captioning model for ingestion. Default `gpt-4o`. Tune to `gpt-4.1-mini` for slightly cheaper, or `gpt-5-mini` if you specifically want reasoning-grade captions (warning: ~10├ù slower) | `gpt-4o` |
...
| `EMAIL_HOST_PASSWORD` | prod-only | SMTP password / API key | `SG.xxxxΓÇª` |
| `EMAIL_USE_TLS` | optional | `true` (default) for port 587 STARTTLS | `true` |
| `EMAIL_USE_SSL` | optional | `true` for port 465 implicit TLS ΓÇö mutually exclusive with `EMAIL_USE_TLS` | `false` |
| `EMAIL_TIMEOUT` | optional | Seconds to wait for SMTP (default `20`) | `20` |
| `DEFAULT_FROM_EMAIL` | yes | From address on every outbound email ΓÇö must match a verified sender on your email provider | `"qp-gen <no-reply@yourdomain.com>"` |
...
ΓÇö any other hit is a production bug.
...
real delivery happens ΓÇö this is the most common reason teachers report
...
* `0003_remove_pdfsource_file_fields` ΓÇö drops the legacy `file` /
  `content_type` columns if Django state had them tracked. Idempotent
  via `DROP COLUMN IF EXISTS`.
* `0004_restore_pdfsource_content_type` ΓÇö **Cluster B fix**.
...
`generation` ΓÇö none changed this round.
...
### A. Editor mount / unmount ΓÇö Cluster A
...
Regression coverage: `frontend/scripts/test-todom-shape.mjs` ΓÇö
...
### B. PDF source upload ΓÇö Cluster B
...
- [ ] Inspect the row: `SELECT id, name, content_type, status FROM
      pdf_source ORDER BY "createdAt" DESC LIMIT 1;` ΓÇö `content_type`
...
Regression coverage: `backend/apps/documents/tests.py` ΓÇö
...
### D. PDF size ΓÇö Cluster C

- [ ] Same paper, ~10 pages.
- [ ] `ls -lh exam-paper.pdf` ΓåÆ record actual size in megabytes.
- [ ] Expected: **< 5 MB** for ~10 pages. (Pre-fix benchmark from the
      brief: 90 MB / 8 pages ΓåÆ expect ~11 MB/page. Post-fix
      expectation: 0.2ΓÇô0.5 MB/page.)
...
- [ ] If figures are missing, the `fetch()` in `loadFigureBytes`
      ...
      `/media/...` requests during export: 4xx/5xx ΓçÆ media route
      misconfigured (nginx or CDN), CORS error ΓçÆ FE/BE are on
      different origins and `Access-Control-Allow-Origin` isn't set
      on the media path.

### F. Answer-script generation ΓÇö Cluster D item 2
...
ΓÇö passing as of this round.

### G. Expired-session flash ΓÇö Cluster D item 3
...
- [ ] Expected: the layout shell may render briefly (header,
      ...
      `/login` fires.
- [ ] If user-scoped content does flash, file a bug ΓÇö the children
