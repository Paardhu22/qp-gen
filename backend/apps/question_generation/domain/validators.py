"""
Question Generation Domain - Validation Framework
"""

from typing import Any, Dict, List

from .datatypes import ExamBlueprint, ValidationError, ValidationReport
from .enums import AcademicClass, BloomsLevel, StreamType, ValidationSeverity
            ...
            if sec.marks_per_question < last_marks:
                errors.append(
                    ValidationError(
                        ...
                        error_message=(
                            f"Section {sec.section_id} marks ({sec.marks_per_question}) < prior ({last_marks})."
                        ),
...
class StreamBalanceRule(IValidationRule):
    def execute(self, blueprint: ExamBlueprint, target_data: Dict[str, Any]) -> List[ValidationError]:
        ...
        targets = getattr(blueprint, "stream_distribution_target", None) or getattr(
            blueprint, "stream_distribution", {}
        )
...
class ValidationOrchestrator:
    ...
    def validate(self, blueprint: ExamBlueprint, target_data: Dict[str, Any]) -> ValidationReport:
        ...
        all_rules = (
            self.structure_validators
            + self.curriculum_validators
            + self.board_validators
            + self.psychometric_validators
            + self.accessibility_validators
            + self.hallucination_validators
        )
            ...
            try:
                ...
                all_errors.append(
                    ValidationError(
                        ...
                        error_message=f"Validation crashed: {exc}",
                        suggested_remediation="Review rule logic.",
                    )
                )

        report.errors = all_errors
        report.is_valid = not any(e.severity == ValidationSeverity.ERROR for e in all_errors)
