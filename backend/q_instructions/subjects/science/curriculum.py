"""
AOS Science — Curriculum Graph & Concept Intelligence
========================================================
Implements Directed Acyclic Graph for concept dependencies,
topological sorting, difficulty estimation, and prerequisite pacing.
"""
...
from q_instructions.core.enums import (
    AcademicClass, StreamType, AbstractionLevel, RelationshipType, BloomsLevel
)
from q_instructions.core.datatypes import (
    ConceptNode, ConceptEdge, ConceptWeightProfile, ChapterMetadata
)
from q_instructions.core.exceptions import ConceptNotFoundError, CyclicDependencyError
...
class ConceptGraph:
    """Directed Acyclic Graph managing concept prerequisites."""
    ...
    def detect_cycles(self) -> bool:
        """Detects circular prerequisite chains using DFS."""
        visited: Set[str] = set()
    ...
    def topological_sort(self) -> List[str]:
        """Returns concepts in prerequisite-safe order using Kahn's algorithm."""
    ...
    def get_prerequisites(self, concept_id: str) -> List[str]:
        """Returns all concepts that are prerequisites for the given concept."""
        prereqs: List[str] = []
        for edge in self.edges:
    ...
    def bfs_distance(self, source: str, target: str) -> int:
        """Computes shortest path distance between two concepts."""
...
# ---------------------------------------------------------------------------
# Chapter Metadata Engine
# ---------------------------------------------------------------------------

class ChapterMetadataEngine:
    """Manages academic chapter profiles."""
...
# ---------------------------------------------------------------------------
# Curriculum Weightage Registry
# ---------------------------------------------------------------------------

class CurriculumWeightageRegistry:
    """Manages concept-level board weightage and competency mappings."""
...
# ---------------------------------------------------------------------------
# Difficulty Estimation Engine
# ---------------------------------------------------------------------------

class DifficultyEstimationEngine:
    """Estimates question difficulty from concept graph properties."""
    ...
    def estimate_difficulty(self, concept_id: str, bloom_level: BloomsLevel) -> float:
        """Returns a normalized difficulty score [0.0, 1.0]."""
        node = self.graph.nodes.get(concept_id)
        if not node:
            return 0.5

        bloom_weights = {
            BloomsLevel.REMEMBER: 0.15, BloomsLevel.UNDERSTAND: 0.30,
            BloomsLevel.APPLY: 0.50, BloomsLevel.ANALYZE: 0.70,
            BloomsLevel.EVALUATE: 0.85, BloomsLevel.CREATE: 0.95,
        }
...
# ---------------------------------------------------------------------------
# Prerequisite Pacing Optimizer
# ---------------------------------------------------------------------------

class PrerequisitePacingOptimizer:
    """Ensures questions appear in prerequisite-safe topological order."""

    def __init__(self, graph: ConceptGraph) -> None:
        self.graph = graph

    def optimize_sequence(self, concept_ids: List[str]) -> List[str]:
        """Orders concepts respecting prerequisite chains."""
...
class CurriculumGraphFactory:
    """Constructs the comprehensive Class 10 Science concept graph."""
